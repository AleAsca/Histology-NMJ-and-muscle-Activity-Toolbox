...
% Version: 1.1.0
%
% *Description*: Analyses the histology slides tiff format and scales them
...
% folder.
%
% *REQUIREMENTS*: This code requires the Bresenham function for length estimation
...
umpx = 2.2963; % conversion from um to px might depend from pic to pic. Number of pixels per um

%% Import images
fprintf("IMPORTING IMAGES... \n");
files = struct2cell(dir(fullfile(dir_in, '*.tif'))); % finds all the .tiff files in the images folder
...
for i = 1:nim % loop through the images
    ...
    a = read(Tiff(fullfile(dir_in, files{1, i})));
